...
result_forplugin$rel_MSE_plugin <- result_forplugin$MSE_plugin/result_forplugin$plugin_estimate
result_forplugin$rel_MSE_weighted <- result_forplugin$MSE_weighted/result_forplugin$weighted_estimate
...
weight_vsplugin2 <- ggplot(data=result_forplugin, aes(x=prop_sample_size)) +
  geom_line(aes(y=abs(bias_plugin), colour=graph_stat)) +
  scale_y_log10(breaks=c(0.001,0.01,0.1,1,10,100,1000)) +
  scale_x_log10(breaks=c(0.05,0.1,0.5,1,5,10)) +
  # xlim(c(0.05,20)) +
  # ylim(c(1e-3,1000)) +
  geom_line(aes(y=abs(bias_weighted), colour=graph_stat), linetype="longdash")
